<?php
/*
Template Name: Search Page
*/
$cat_id = get_queried_object_id();
?>
<?php get_header() ?>
<!-- Begin Site Title
================================================== -->
<div class="container">
    <div class="mainheading"> </div>

    <section class="featured-posts">
        <div class="section-title">
            <?php if ( have_posts() ) : ?>
                <h2><span><?php printf( __( 'Search Results for: %s', 'helsinki' ), get_search_query()); ?></span></h2>
            <?php else : ?>
                <h2><span><?php _e( 'Nothing Found', 'helsinki' ); ?></span></h2>
            <?php endif; ?>
        </div>
        <div class="card-columns listfeaturedtag">


            <?php
            if ( have_posts() ) :
                /* Start the Loop */
                while ( have_posts() ) : the_post();
                ?>

                    <!-- begin post -->
                    <div class="card">
                        <div class="row">
                            <div class="col-md-5 wrapthumbnail">
                                <?php if (has_post_thumbnail()) : ?>
                                    <a href="<?php the_permalink(); ?>">
                                        <div class="thumbnail" style="background-image:url(<?php the_post_thumbnail_url() ?>);">
                                        </div>
                                    </a>
                                <?php endif; ?>
                            </div>
                            <div class="col-md-7">
                                <div class="card-block">
                                    <h2 class="card-title">
                                        <a href="<?php the_permalink(); ?>">
                                            <?php the_title(); ?>
                                        </a>
                                    </h2>
                                    <h4 class="card-text">
                                        <?php echo substr(get_the_excerpt(), 0,120);  ?>
                                    </h4>
                                    <div class="metafooter">
                                        <div class="wrapfooter">
								<span class="meta-footer-thumb">
								<a href="<?php the_author_posts() ?>">
                                    <img class="author-thumb" src="<?php echo get_avatar_url( get_the_author_meta( 'ID' )); ?>" alt="<?php the_author() ?>">
                                </a>
								</span>
                                            <span class="author-meta">
								<span class="post-name">
                                    <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) ); ?>">
                                        <?php the_author(); ?>
                                    </a>
                                </span><br/>
								<span class="post-date"><?php echo get_the_date('F j, Y'); ?></span>
                                        <span class="dot"></span>
                                        <span class="post-read">
                                           In <?php echo helsinki_post_taxonomy(get_the_ID(), 'category'); ?>
                                        </span>
								</span>
                                            <span class="post-read-more">
                                        <a href="<?php the_permalink(); ?>" title="Read Story">
                                            <svg class="svgIcon-use" width="25" height="25" viewbox="0 0 25 25">
                                                <path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd">
                                                </path>
                                            </svg>
                                        </a>
                                    </span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- end post -->
            <?php

                endwhile; // End of the loop.

                the_posts_pagination( array(
                    'prev_text' => '<<' . '<span class="screen-reader-text">' . __( 'Previous page', 'helsinki' ) . '</span>',
                    'next_text' => '<span class="screen-reader-text">' . __( 'Next page', 'helsinki' ) . '</span>' . '>>',
                    'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'helsinki' ) . ' </span>',
                ) );

            else : ?>

                <p><?php _e( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'helsinki' ); ?></p>
                <?php
                get_search_form();

            endif;
            ?>
        </div>
    </section>
    <?php get_footer() ?>
</div>
<!-- /.container -->